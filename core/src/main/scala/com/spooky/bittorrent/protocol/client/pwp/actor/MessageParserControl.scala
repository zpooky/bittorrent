...
class MessageParserControl(connection: ActorRef, actor: Actor)(keyPair: MSEKeyPair, sessions: Sessions)(implicit context: ActorContext) {
  ...
  private def handle(handshake: Handshake): Unit = sessions.get(handshake.infoHash) match {
    case Some(session) => {
