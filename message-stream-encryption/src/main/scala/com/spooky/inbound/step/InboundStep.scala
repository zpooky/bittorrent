...
class InboundStep(skey: InfoHash) extends PublicKeyBase with InStep {

  //  def step(reply: Reply)(in: ByteString): Step = inbound(in)(reply)

  def step(in: ByteString, reply: Reply): SendPublicKeyStep = {
    ...
    new SendPublicKeyStep(skey, publicKey, remotePublicKey, secretKey)
  }
}

object InboundStep {
  def main(args:Array[String]) {
    val is = new InboundStep(InfoHash.hex("597f6a218a58b0fe7880ba12466ccd89ca6c778f"))
